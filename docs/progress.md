...
**마지막 갱신:** 2026-06-13 (1차 구조 개선 완료 + 운영 피드백 반영 중. **예약 문자 버그 수정·배포**: 리마인드·복습이 단건 `/messages/v4/send`의 scheduledDate 미지원으로 `failed`였던 것을 `/messages/v4/send-many/detail`+ISO8601(+09:00)로 교체. 어드민 기본 시간 13:00~16:00. 진행 중: ② 접수 문자 선착순/대기 2분기(문구 확정 대기), ③ 본인 예약 조회 페이지(설계안 보고·결정 대기). 재검증 필요: 새 신청→결제완료→현황판 '예약 완료' + Solapi 예약대기건 2건.)
