...
> 🆕 **대기 칸 '만석' 명시(2026-06-24)**: 고객 캘린더 대기 상태 칸이 "대기 가능"만 보여 자리가 찼다는 게 안 와닿는다는 피드백 → 첫 줄에 **"만석"**(굵게, `text-ink-700`) 추가하고 둘째 줄 **"대기 가능"**(대기 인원 있으면 `대기 가능 N명`, 없으면 `대기 가능`). 기존 "N명 대기중" 표현을 "만석 / 대기 가능 N명" 구조로 교체. `classAnchorHtml` 대기 분기만 수정(프론트만, 배포 불필요·Pages 자동). 계약 테스트 18개 PASS.
