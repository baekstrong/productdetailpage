...
**마지막 갱신:** 2026-06-07 (지난 수업 처리: 고객 달력 자동 숨김+예약 차단, admin '종료' 표시 / admin 섹션 순서 재배치)
...
- **지난 수업 자동 처리(프론트 계산, 시작 시각 기준)** (`index.html` + `admin.html`)
  ...
  - 관리자: 드롭다운 라벨 끝 `· 종료` 표기, 수업 목록 표는 해당 행 muted + `종료` 배지(기록·정리용으로 행/버튼은 유지).
  - DB status 변경/cron 없음(YAGNI). 관리자 드롭다운 기본 선택은 여전히 첫 수업(추후 '다음 수업' 기본화 검토 가능).
