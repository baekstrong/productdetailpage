...
> 🆕 **중복 신청 차단을 '수업별' → '번호 전체'로 확대 + 관리자 중복 배너(2026-07-04)**: 같은 번호가 **다른 날짜**에 또 신청하는 것을 못 막던 문제(박흥수 01090013630이 6번/4번 두 수업 동시 신청). 기존 차단은 `(class_id, phone)` 기준이라 다른 수업이면 통과했음. ① `submit-reservation`: 중복 조회에서 `class_id=eq.` 제거 → `phone=eq.…&reservation_status=not.in.(cancelled,no_show)` 전체 기준, 있으면 409(문구 상수 `DUPLICATE_MESSAGE`="이미 신청하신 수업이 있습니다. 한 번호로는 한 수업만…"). ② `schema.sql`: 유니크 인덱스 `reservations_active_unique`를 `(class_id, phone)`→**`(phone)`** where 활성. ③ `admin.html`: 전체 예약(`adminData.reservations`)을 번호별로 묶는 `activeReservationsByPhone()` — 예약 목록 위 **빨강 배너 `#dup-phone-alert`**(2건 이상 번호 = 이름·번호·날짜들·건수 나열, 남길 1건 외 취소 유도) + 각 행 번호 옆 `중복 N건` 배지. 계약 테스트 20개 PASS(회귀 계약 6줄 추가) + 집계 로직 node 자체검증. **⚠️ 배포 대기 2가지: (a) `supabase functions deploy submit-reservation --no-verify-jwt`(안 하면 신규 중복 여전히 통과) (b) 운영 DB 인덱스 교체 — 기존 중복이 있어 바로 못 걸므로 순서: admin 배너로 중복 확인 → 남길 1건 외 취소 처리 → `drop index if exists public.reservations_active_unique;` → schema.sql의 새 `create unique index … (phone)` 실행.** admin.html은 push로 Pages 자동(배너/배지 먼저 보임 → 기존 중복 정리에 사용).**
>
