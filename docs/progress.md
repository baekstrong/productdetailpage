...
> 🆕 **'선착순 확보(결제 링크 X)' 버튼 신설(2026-07-07)**: 선착순 취소로 여석이 생겼는데 **수업이 아직 한참 남은** 대기자에게 자리를 넘길 때, 결제 링크를 지금 주면 안 됨(결제는 D-7). 그래서 **결제 링크 없이 '자리 확보'만 통보**하는 버튼·문자 신설. 상태는 **새 status 없이 `payment_target` 재사용**(자리 점유·정원 계산·D-7 리마인더 대상 로직 그대로) + `payment_status='pending'` 유지(24h 결제 시계 안 돌림, `seat_opened`/`payment 안내`는 `sent`로 저장돼 구분됨). 변경 4파일: ① `solapi-reservations` 새 타입 **`seat_secured`** 템플릿(결제 링크 없음, "자리 확보했고 결제 링크는 1주일 전에" 안내) ② `admin-reservations.updateReservation` payment_target 분기에서 `notify==='seat_secured'`면 링크 빼고 발송 + `RESENDABLE_TYPES` 추가 ③ `admin.html` 벌크 버튼 "선착순 확보 (결제 링크 X)"(`data-bulk-action="seat-secured"` → `payment_target`+`pending`, `notify:'seat_secured'`, 확인창) + 배지 분기: `payment_target`+`sent`→"결제 안내 중"(기존), `payment_target`+`pending`→**"선착순 확보 N"**(신청순 좌석번호, `groupBadgeHtml`/`seatRankLabel` 양쪽) + 현황판 "자리 확보 문자" 행 ④ 계약 테스트 무변경(20 PASS). 좌석번호/배지 로직 node 자체검증 통과. **이후 흐름:** D-7 때 백관장이 기존 "결제 안내 대상으로 지정" 누르면 `sent`+진짜 결제 링크(`payment 안내`) 발송으로 자연 연결. **⚠️ 배포 대기: `supabase functions deploy solapi-reservations --no-verify-jwt` + `… admin-reservations --no-verify-jwt`(admin.html은 push로 Pages 자동). 미배포 시 버튼 눌러도 `seat_secured` 문자 안 나감.**
>
