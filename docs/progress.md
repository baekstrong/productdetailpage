...
> 🆕 **고객 캘린더 칸 상태 단순화(2026-06-22)**: `index.html` 공개 캘린더 날짜 칸이 "가능 N / 대기 N"을 동시에 보여줘 헷갈린다는 피드백 반영 → 칸당 **상태 한 줄만** 표시. 예약 가능(`available>0`)이면 **"예약 가능 / N자리 남음"**(brand 색), 정원 차면 **"대기 가능 / N명 대기중"**(회색). 시간 배지는 그대로. `classAnchorHtml` 예약가능 분기만 수정(프론트만, 배포 불필요·Pages 자동). 계약 테스트 18개 PASS.
